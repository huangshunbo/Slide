...
public class LeViewDragHelper {
    private static final String TAG = "LeViewDragHelper";
     ...
     */
    public static LeViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new LeViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static LeViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final LeViewDragHelper helper = create(forParent, cb);
     ...
     * @param forParent Parent view to monitor
     */
    private LeViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    public boolean settleViewAt(View capturedView, int finalLeft, int finalTop, final int duration) {
        if (mDragState == STATE_SETTLING) {
            return false;
        }
        ...
        capturedView.offsetLeftAndRight(1);
        setDragState(STATE_DRAGGING);
        if (dx == 0 && dy == 0) {
    ...
    public boolean slideView(View capturedView, int startLeft, int finalLeft, final int duration) {
        mCapturedView = capturedView;
        final int startTop = capturedView.getTop();
        final int dx = finalLeft - startLeft;
        capturedView.offsetLeftAndRight(startLeft);
        ...
        mScroller.startScroll(startLeft, startTop, dx, 0, duration);
    ...
    public boolean continueSettling(boolean deferCallbacks) {
        if (mDragState == STATE_SETTLING) {
            if (mCapturedView == null) {
                if (deferCallbacks && mParentView != null) {
    ...
    private void clearMotionHistory(int pointerId) {
        if (mInitialMotionX == null || pointerId == -1 || pointerId >= mInitialMotionX.length) {
            return;
        }
...
//                    float oldScrollX = mCapturedView.getScrollX();
//                    float scrollX = oldScrollX - (x - mLastMotionX[mActivePointerId]);
//                    mCapturedView.scrollTo((int)scrollX, 0);
//
//                    if (idx != 0) {
//                        mCallback.onViewPositionChanged(mCapturedView, -(int)scrollX, 0,
//                                idx, 0);
//                    }
                    saveLastMotion(ev);
                } else {
